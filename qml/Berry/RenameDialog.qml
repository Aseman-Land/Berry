...
Item {
    ...
    onSourceChanged: name_input.text = Berry.fileName(source)
    ...
    Column {
        ...
        Button {
            ...
            onClicked: rename_dialog.accept()
        }
    }
    ...
    function accept() {
        Berry.renameFile(source,name_input.text)
        rename_dialog.successfully()
        hideSubMessage()
    }
    ...
    LanguageSwitcher {
        onRefresh: {
            title_lbl.text = qsTr("Rename Dialog")
            name_placeholder.text = qsTr("Enter new name")
            create_btn.text = qsTr("Rename")
        }
    }
}
